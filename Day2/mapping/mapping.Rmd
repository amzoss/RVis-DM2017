...
point_locations <- read_csv("Calaveras-County-Government-Offices.csv")
...
map_calaveras <- geom_polygon(data = county_map[county_map$subregion=="calaveras",], aes(x = long, y = lat), fill=cal_county_color, color = cal_county_border, size=1)

map_points <- geom_point(aes(x=point_locations$Longitude, y=point_locations$Latitude), color=dot_color, size=dot_size, alpha=dot_transparency)
...
elnino <- read_csv("../el nino/elnino.csv", na=c(".",NA), 
